...
\begin{lemma}
%$N=N_{opt}$ happens if and only if there is no infectious nodes in the network, i.e. all nodes in $N=N_{opt}$ are in state G.
$N=N_{opt}$当且仅当网络中不存在传染节点时发生，即所有属于集合$N$中的节点，都处于状态G。
\label{lemma:no_infectious}
\end{lemma}
\begin{proof}
%Since that the number of all tickets in the network is $|N_{opt}|$, if there exist an infectious node $n_a$, then its tickets must be at least 2 and the tickets in all other nodes are less than $|N_{opt}|-2$. So the number of nodes holding the message is at most $|N_{opt}|-1$, and in this case $n_a$ has 2 tickets for the message, and all the other nodes with replicas only have 1 ticket. So the ``only if'' condition works. 
%The node set holding the message accords with $N_{opt}$ means that the number of nodes holding the message is $|N_{opt}|$, which is the maximal number of replicas for this message in the network. So the number of tickets in each of these nodes is equal to 1. According to the definition of node state in Tabu-MPAR, all of these nodes are in infected state, and all other nodes are in pure state, which indicates that currently there is no infectious node in the network. So the ``if'' condition works.
...
\begin{lemma}
%If the set $N$ evolves to $N_{opt}$, then it would not change any more.
...
\end{theorem}
\begin{proof}
%The state transition between B and G does not modify the set $N$. However, from line 4--10 in algorithm~3, it is guaranteed that the distributed tickets are no less than 1, which means that any node in state B would transfer to state G if there are enough encounter opportunities during the routing process. So we just need to focus on the transitions between W and G, as shown in grid WG and GW, where the rule is to transfer the node belonging to $N_{opt}$ to G. Consequently our theorem comes directly from lemma~2 and 3.
...
\begin{figure}[!t]
\centering
\includegraphics[width=3.5in]{paper-MPAR/Manhattan}
...
在仿真中，共产生30,000条消息，对于每条消息，其源节点和目的节点从所有行人节点中随机选择。每条消息的大小设为500 K 至 1 M 之间。仿真区域的大小根据节点数目的多少按同比例设定。对于整个仿真过程，其仿真器时间设为12天。

本节将MPAR算法与两个经典算法相比较：Delegation Forwarding (DF) \cite{Erramilli2008}与Simbet \cite{Daly2007}。采用四项评估指标：投递率(delivery ratio)，平均时延(average latency)，网络开销（overhead ratio）及平均每跳数（average hop count）。仿真结果表明，MPAR算法可以较大程度上提升路由性能表现，尤其在于消息投递率。



\subsection{自变量：消息生存时间}

在将消息生存时间设为自变量时，节点缓存大小固定在200M。消息的生存时间从10小时到30小时变化，仿真结果如\figurename~\ref{fig:chap3_delivery_ttl}，\figurename~\ref{fig:chap3_latency_ttl}，\figurename~\ref{fig:chap3_overhead_ttl}，\figurename~\ref{fig:chap3_hop_ttl}。

\begin{figure*}[!t]
\centering
\subfigure[Number of nodes: $|\overline{N}|=200$\label{200_delivery_ttl}]
{\includegraphics[width=0.45\textwidth]{paper-MPAR/200_delivery_ttl}}
\subfigure[Number of nodes: $|\overline{N}|=400$\label{400_delivery_ttl}]
{\includegraphics[width=0.45\textwidth]{paper-MPAR/400_delivery_ttl}} \\
\subfigure[Number of nodes: $|\overline{N}|=600$\label{600_delivery_ttl}]
{\includegraphics[width=0.45\textwidth]{paper-MPAR/600_delivery_ttl}}
\subfigure[Number of nodes: $|\overline{N}|=800$\label{800_delivery_ttl}]
{\includegraphics[width=0.45\textwidth]{paper-MPAR/800_delivery_ttl}}
\caption{Performance comparisons of delivery probability vs. time-to-live}
...
{\includegraphics[width=0.45\textwidth]{paper-MPAR/200_avgLatency_ttl}}
\subfigure[Number of nodes: $|\overline{N}|=400$\label{400_avgLatency_ttl}]
{\includegraphics[width=0.45\textwidth]{paper-MPAR/400_avgLatency_ttl}} \\
\subfigure[Number of nodes: $|\overline{N}|=600$\label{600_avgLatency_ttl}]
{\includegraphics[width=0.45\textwidth]{paper-MPAR/600_avgLatency_ttl}}
\subfigure[Number of nodes: $|\overline{N}|=800$\label{800_avgLatency_ttl}]
{\includegraphics[width=0.45\textwidth]{paper-MPAR/800_avgLatency_ttl}}
\caption{Performance comparisons of average end-to-end latency vs. time-to-live}
...
{\includegraphics[width=0.45\textwidth]{paper-MPAR/200_overhead_ttl}}
\subfigure[Number of nodes: $|\overline{N}|=400$\label{400_overhead_ttl}]
{\includegraphics[width=0.45\textwidth]{paper-MPAR/400_overhead_ttl}} \\
\subfigure[Number of nodes: $|\overline{N}|=600$\label{600_overhead_ttl}]
{\includegraphics[width=0.45\textwidth]{paper-MPAR/600_overhead_ttl}}
\subfigure[Number of nodes: $|\overline{N}|=800$\label{800_overhead_ttl}]
{\includegraphics[width=0.45\textwidth]{paper-MPAR/800_overhead_ttl}}
\caption{Performance comparisons of overhead ratio vs. time-to-live}
...
{\includegraphics[width=0.45\textwidth]{paper-MPAR/200_avgHop_ttl}}
\subfigure[Number of nodes: $|\overline{N}|=400$\label{400_avgHop_ttl}]
{\includegraphics[width=0.45\textwidth]{paper-MPAR/400_avgHop_ttl}} \\
\subfigure[Number of nodes: $|\overline{N}|=600$\label{600_avgHop_ttl}]
{\includegraphics[width=0.45\textwidth]{paper-MPAR/600_avgHop_ttl}}
\subfigure[Number of nodes: $|\overline{N}|=800$\label{800_avgHop_ttl}]
{\includegraphics[width=0.45\textwidth]{paper-MPAR/800_avgHop_ttl}}
\caption{Performance comparisons of average hop count vs. time-to-live}
